...
// (functions/api/scores.ts) with only D1 stubbed. In the Free-tier async-audit version, this handler
...
// real handler for the submission form's Unicode-input case,
...
describe('POST /api/scores submitterToken validation', () => {
  ...
  // An explicit null ATTACHES the field, so it is a
...
describe('POST /api/scores self-replacement batch', () => {
  ...
  // The retry batch's cutoff must be evaluated exactly once. The clock moves
