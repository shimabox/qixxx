...
// (functions/api/ranking/[id]/replay.ts) rather than a mocked response.
// This is an integration test of the shipped handler behavior.
// Only D1 itself is stubbed — the version comparison, the status
 ...
 * lookup is by id alone now, because pending rows are
 * servable too, so every status/age/version decision is the handler's own and
...
describe('GET /api/ranking/:id/replay', () => {
  ...
  it('keeps the row: a 410 issues no DELETE/UPDATE, only the SELECT', async () => {
    // A 410 response must retain the row; no DELETE or UPDATE is allowed.
  ...
  // Exercise the complete acceptance matrix.
