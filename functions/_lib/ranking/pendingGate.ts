// Two shared boundaries for the Free-tier async-audit flow (docs/plans/2026-
// 08-19-ranking-free-async):
//
//   1. The 24h pending freshness boundary (spec item 5's "24時間境界の統一
//      定義") — one definition, used by every reader and writer.
//   2. The verified-10th-place submission gate (spec items 2 and 6) — the
//      ONLY basis for whether a submission is accepted.
//
// Kept in one module so no call site can drift on either boundary.
import type { Env } from '../types';

/** The pending freshness window (spec item 5's 24h). */
export const PENDING_EXPIRY_MS = 24 * 60 * 60 * 1000;

/**
 * The ONE cutoff every pending freshness/expiry decision is made against
 * (spec item 5's "24時間境界の統一定義"): given a "now" in epoch
...
/** Spec item 5's expiry half of the boundary above, for the JS-side callers (`created_at <= cutoff`). */
 ...
 * (spec items 2 and 6's confirmed `COALESCE(10位スコア, -1)` rule): the
 ...
 * submissions could lock every honest player out of the ranking (spec item
 * 6's role separation / the anti-griefing tests).
 *
...
/** Spec items 2/6's confirmed comparison: the score must STRICTLY exceed the threshold — a tie with 10th place is out of range (first-come-first-served favors the incumbent already in `verified`). */
