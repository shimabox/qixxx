...
// -19-ranking-free-async task 9. Covers the two field families the Free-tier
...
// into one board, spec item 5's 2026-08-20 revision), and the fact that the
              ...
              if (/status = 'pending'/.test(sql)) {
                ...
                const filtered = rows.filter(
                  (r) =>
                    ...
                    // Spec item 5's shared boundary: FRESH is created_at > cutoff.
                    r.created_at > cutoff
                );
                return { results: sorted(filtered).slice(0, limit) };
              }
...
describe('GET /api/ranking', () => {
  ...
  // docs/plans/2026-08-19-ranking-free-async spec item 5 (2026-08-20 revision).
  describe('displayEntries (the merged board)', () => {
    ...
    it('shows pending rows unconditionally when fewer than 10 verified rows exist (no threshold filtering on the display side)', async () => {
      ...
      // gate's job (spec item 6's role separation).
  ...
  // Spec item 6: the submission basis must not move when pending rows appear.
