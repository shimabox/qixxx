...
// ranking-free-async spec items 5 and 6, as revised 2026-08-20). Returns TWO
...
//     tagged `status`. Purely presentational: spec item 6's role separation
//     says the merged view influences neither the pre-gate nor the atomic
...
/**
 * How many pending rows may be CANDIDATES for the merged board (spec item
 * 5's confirmed "表示 pending 候補の上限3件").
 *
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  // Display-pending candidates (spec item 5): current season/ruleset, FRESH
  ...
  // threshold is the submission gate's business (spec item 6's role
  // separation), and a stored pending row is shown for what it is — the merge
