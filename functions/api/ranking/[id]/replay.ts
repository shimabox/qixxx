// GET /api/ranking/:id/replay returns the seed, RLE input list, and version
// information needed to replay a ranked run, plus the row's audit status.
// The status is operational metadata and is not rendered by the viewer.
// Pending and verified replays look identical; status is returned only for
// operations and debugging, such as checking from a curl whether a row has
...
// -> 200 + status
//
// The fixed order resolves an ambiguity: an expired pending row at an
...
// Pending rows are part of the merged board now, so refusing to
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  // Gone, while leaving the stored row untouched; only this endpoint refuses
  // to serve it.
  const isCurrent =
  ...
  // and debugging. The viewer plays a pending run exactly like a verified
  // one and does not render the status.
