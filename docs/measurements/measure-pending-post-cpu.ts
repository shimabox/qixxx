// CURRENT LOCAL IN-PROCESS WALL-CLOCK BENCHMARK for the asynchronous-audit
// version of POST /api/scores. It uses the pending-POST contract:
...
// + canonical re-encode + SHA-256). D1 is an in-memory mock, so this is a
// local reference figure rather than a Cloudflare CPU-time measurement.
//
