...
## 2. KV ネームスペース作成とバインド（シェア機能に必須）

1. **Workers & Pages** → **KV** → **Create namespace** → 名前は例: `qixxx-shares`
2. Pages プロジェクト → **Settings** → **Bindings**（または Functions） → **KV namespace bindings** → **Add**
   - **Variable name**: `SHARES`（コードがこの名前を参照。変更不可）
   - **KV namespace**: 手順 1 で作ったもの
3. Production / Preview 両方の環境にバインドする（プレビューは同じ namespace でも別でも可）
4. バインド追加後は **再デプロイ**が必要（Deployments → Retry deployment で可）
...
- 本番の KV バインドは上記手順 2 のダッシュボード設定が正で、リポジトリの `wrangler.toml` の KV 設定は**ローカル開発専用**（コメント参照）。
