...
現在の値は 300(`src/config.ts`)。サーバーはリプレイ検証中に
**上限を 1 回超えたクレームを検出した時点で即時拒否**する。DoS 対策で
監査 1 行あたりの CPU を抑えるための数値であり、ゲームのルールではない。

値の根拠と計測は `docs/ranking-cpu-measurement.md` §7。要点:
小刻みに陣地を刻むボットは失敗した 1 ステージだけで約 52 クレームに達し、
丁寧な複数ステージ攻略はさらに積み上がるので、旧値 100 は正規プレイに近すぎた。
300 は正規プレイに余裕を持たせつつ、監査は最悪でも 1 行あたり約 3.8 秒
(約 12.5ms/クレーム)、満杯 200 行で約 3 監査サイクルに収まる。
なお、実行可能なリプレイが run 全体の 10800 tick 内で到達できるクレーム数は
実構成でおよそ 230 が上限なので、300 は現実のリプレイが届く限界の上に置いた
バックストップとして働く。

**変更手順:**

- [ ] **公開前(`scores` が空)なら定数を書き換えるだけ**でよい。まだ verified 行が
      存在しないので、シーズン切替は不要(今回の 100→300 はこの経路)
- [ ] **公開後に緩める(増やす)場合は、シーズンまたは検証ルールの変更として扱うこと。**
      それまで拒否されていた入力列が通るようになり、同一シーズン内で
      「拒否されたプレイヤー」と「通ったプレイヤー」が**異なる検証ルールで
      同じランキングに並ぶ**ことになる。したがって `CURRENT_SEASON_ID` を +1 する
- [ ] いずれの場合も、上げるときは CPU コストを再計測すること
      (上限を上げる = 攻撃入力の上界が上がる)。ハーネスは
      `docs/measurements/measure-claim-cap.ts`、方法論は `docs/ranking-cpu-measurement.md` §7
...
加えて migration 0005 以降は実効 seed `rng_key = deriveStageSeed(seed, 1)` も UNIQUE なので、
**公開済みリプレイを取得して入力列だけ微改変した再投稿**は、同じ seed の 2 行目として
INSERT 時に 409 で弾ける。

seed を変えた複製も同じ制約で弾ける。ステージごとの乱数は
`deriveStageSeed(seed, stage)`(`"<seed>:<stage>"` の FNV-1a)から作るため、
`"<seed>:"` までのハッシュ状態が一致する 2 つの seed は全ステージで同じ乱数系列になる
(例: `1485211075` と `2522981067`)。このような seed の組は seed も `replay_hash` も
異なるので、数値 seed の一意性では拒否できない。FNV-1a の更新は全単射なので
「`rng_key` が等しい ⇔ 全ステージの乱数系列が等しい」が成り立ち、`rng_key` の一意性なら
同じ盤面になる別 seed を入力列に関係なく拒否できる。
正当なプレイ同士がこれに当たるのは盤面が完全に同一になる場合だけで、確率は seed 衝突と同じ
組ごとに 2^-32 のオーダー。
...
これは v1 の既知の限界として受け入れる。

また `MAX_VERIFIED_CLAIMS`(300)を超えたプレイは監査で削除されるが、クライアントは
ゲームオーバー時点のクレーム数を数えて、上限超えのプレイには投稿フォームを出さず
理由だけを表示する(`src/ui/ranking.ts` の `decideSubmissionOffer`)。上限は
2026-09-13 に計測して 100 から 300 に引き上げ済み(§4.3、`docs/ranking-cpu-measurement.md` §7)。
