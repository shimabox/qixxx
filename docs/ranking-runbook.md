...
加えて migration 0005 以降は `seed` も UNIQUE なので、**公開済みリプレイを取得して
入力列だけ微改変した再投稿**は、同じ seed の 2 行目として INSERT 時に 409 で弾ける。

一方で、**本方式でも防げないもの**:

- ボットによる自動プレイ入力
- **同じ乱数系列になる別 seed への複製投稿**。ステージごとの乱数は
  `deriveStageSeed(seed, stage)`(`"<seed>:<stage>"` の FNV-1a)から作るため、
  `"<seed>:"` までのハッシュ状態が一致する 2 つの seed は全ステージで同じ乱数系列になる
  (例: `1485211075` と `2522981067`)。公開リプレイの入力列をそのまま、こうした別 seed で
  投稿すると、seed も `replay_hash` も異なるので一意制約では拒否できず、監査も通る。
  リプレイの同一性を数値 seed ではなく実効乱数系列から導くまでは既知の限界として残す
