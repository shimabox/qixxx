...
/**
 * GET /api/ranking's `displayEntries` is the merged board the UI draws:
 * verified and fresh pending rows share one ordering and representation.
 * Each row carries every RankingEntry field plus `status`.
 */
...
async function mockFullBoardRelativeToLiveScore(
  ...
  await page.route('**/api/ranking', async (route) => {
    ...
    // `displayPending` models the anti-griefing scenario:
 ...
 * load was enough to push a timing-sensitive neighbouring test
 * (tests/e2e/gameover-share.spec.ts, whose polls allow only 8s) over its
 * budget under full-suite contention.
 *
...
test.describe('ranking display', () => {
  test('shows entries with their dates, an X-handle link, a disabled REPLAY for unavailable rows, and renders an HTML-metacharacter name as inert text', async ({ page }) => {
    ...
    // Each row includes its date, score, stage, and name.
  ...
  // To cover narrow-device behavior, a submission made with only an
  ...
  test('a handle-only row shows the handle as its name (and only "(no name)" when there is genuinely neither)', async ({ page }) => {
    ...
    // link: the audit verifies the score, never
  ...
  // ONE board, and one kind of row. A verified or fresh pending entry
  // uses the same representation. A
  ...
  // Identity behavior: a pending row and its verified self are the SAME
...
test.describe('ranking browsing is a Title-screen-only affordance', () => {
  ...
  test('tabbing to RANKING and pressing Enter opens the list without starting a run', async ({ page }) => {
...
test.describe('name-input submission flow', () => {
  ...
  // Doubles as the browser half of the ANTI-GRIEFING pair: pending rows share the board
  ...
  // Switching input modes can make the row ambiguous: "checking USE X HANDLE
  ...
  // A retryable 429 must preserve the submission opportunity. Previously,
  // a 429 hid the SUBMIT
  ...
  // The private-browsing / storage-blocked fallback:
...
test.describe('replay viewing', () => {
  test('plays a mocked replay, skips to the final stage, exits, and never POSTs or touches the stored high score', async ({ page }) => {
    ...
    // the run's LAST stage as such — on stage 1
    ...
    // not disabled. It is also proof the chunked
    ...
    // "終了して一覧へ戻る": EXIT reopens the ranking list rather
  ...
  // While watching a replay, the player needs to
  ...
  // On a run that ended on the stage being
  ...
  // A fresh pending row is replayable and uses the same UI as a verified
  // row: no notice on the board, none in the status line.
  ...
  test('the longest status wording stays on ONE line, and the end overlay puts SCORE and STAGE on their own lines', async ({ page }) => {
    // The narrow layout case: the status line wrapped into a
