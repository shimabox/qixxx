 ...
 * fixed-width run-total countdown from the 3-minute default time limit
 * (`3:00.0`, single-digit minutes for the entire run) rather than an
...
async function setWorstCaseHudText(page: import('@playwright/test').Page): Promise<void> {
  await page.evaluate(() => {
    ...
    if (l2.style.display === 'block') {
      ...
      l3.textContent = 'TIME 3:00.0';
    } else {
      l1.textContent = 'STAGE 999  SCORE: 999999  HI: 999999  TIME 3:00.0  OCCUPANCY: 100%  LIVES: 9  x9';
    }
  });
}
