// Real-D1 migration test for 0003. Applies 0001 + 0002 alone — the schema as it stands
...
describe('migrations/0003_submitter_hash.sql (real local D1)', () => {
  it('adds a nullable submitter_hash that pre-existing rows come out of the migration with as NULL, and indexes the delete-candidate lookup', async () => {
    ...
    // Two rows in the shape a pre-0003 handler wrote: one already verified,
    // one still pending. Neither has a submitter_hash column to fill in at
    // this point.
    wrangler(
      persistDir,
      '--command',
