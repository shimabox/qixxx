...
import { AUDIT_D1_BIND_MODES, auditDatabaseWithBindMode, createTestD1, type TestD1 } from './testSupport/localD1';
import type { AuditD1Database } from './d1Adapter';
import { acquireLock, renewLock, releaseLock, LOCK_FENCE_SQL_FRAGMENT } from './lock';

describe.each(AUDIT_D1_BIND_MODES)('audit_lock (real local D1, %s binds)', (bindMode) => {
  let testDb: TestD1;
  ...
  it('a fresh (never-yet-acquired) database allows the first acquire to succeed immediately', async () => {
    const lock = await acquireLock(auditDb);
  ...
  it('a second acquire attempt fails (returns null) while the first lease is still unexpired', async () => {
    const first = await acquireLock(auditDb);
    expect(first).not.toBeNull();
    const second = await acquireLock(auditDb);
  ...
  it('renewLock succeeds for the current, unexpired owner', async () => {
    const lock = await acquireLock(auditDb);
    const renewed = await renewLock(auditDb, lock!.ownerToken);
    expect(renewed).toBe(true);
  });

  it('renewLock fails for a token that never held the lock', async () => {
    await acquireLock(auditDb);
    const renewed = await renewLock(auditDb, 'not-the-real-owner-token');
  ...
  it('releaseLock succeeds for the current unexpired owner, and immediately frees the lock for a new acquire', async () => {
    const lock = await acquireLock(auditDb);
    const released = await releaseLock(auditDb, lock!.ownerToken);
    expect(released).toBe(true);
    const next = await acquireLock(auditDb);
  ...
  it('renewLock is a no-op for a DIFFERENT, still-live owner (two-run overlap never lets the second run extend the first\'s lease)', async () => {
    ...
    const renewed = await renewLock(auditDb, 'some-other-runs-token');
    const released = await releaseLock(auditDb, 'some-other-runs-token');
    expect(renewed).toBe(false);
    expect(released).toBe(false);
    // And the real owner's lease is untouched by those no-ops.
    expect(await renewLock(auditDb, first!.ownerToken)).toBe(true);
  });
  ...
  describe('LOCK_FENCE_SQL_FRAGMENT (write-statement-level fencing)', () => {
    it('EXISTS(...) is true for the current owner while the lease is live', async () => {
      const lock = await acquireLock(auditDb);
      const row = await auditDb.prepare(`SELECT ${LOCK_FENCE_SQL_FRAGMENT} AS fenced`).bind(lock!.ownerToken).first<{ fenced: number }>();
    ...
    it('EXISTS(...) is false for a token that is simply not the current owner', async () => {
      await acquireLock(auditDb);
      const row = await auditDb.prepare(`SELECT ${LOCK_FENCE_SQL_FRAGMENT} AS fenced`).bind('wrong-token').first<{ fenced: number }>();
      expect(row!.fenced).toBe(0);
    });
  });
});
